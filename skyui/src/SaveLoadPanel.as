dynamic class SaveLoadPanel extends MovieClip
{
    ...
    function ShowScreenshot()
    {
        ...
        {
            var __reg2 = this.SaveLoadList_mc.__get__selectedEntry().name;
            var __reg3 = 20;
            ...
            {
                __reg2 = __reg2 + (", " + this.SaveLoadList_mc.__get__selectedEntry().raceName);
            }
            if (this.SaveLoadList_mc.__get__selectedEntry().level != undefined && this.SaveLoadList_mc.__get__selectedEntry().level > 0) 
            {
                __reg2 = __reg2 + (", " + this.PlayerInfoText.LevelText.text + " " + this.SaveLoadList_mc.__get__selectedEntry().level);
            }
    ...
    {
        var __reg2 = 20;
        for (var __reg3 in this.SaveLoadList_mc.__get__entryList()) 
        {
            if (this.SaveLoadList_mc.__get__entryList()[__reg3].text.length > __reg2) 
            {
                this.SaveLoadList_mc.__get__entryList()[__reg3].text = this.SaveLoadList_mc.__get__entryList()[__reg3].text.substr(0, __reg2 - 3) + "...";
            }
        }
        if (abDoInitialUpdate) 
        {
            var __reg4 = "$[NEW SAVE]";
            if (this.bSaving && this.SaveLoadList_mc.__get__entryList()[0].text != __reg4) 
            {
                var __reg5 = {name: " ", playTime: " ", text: __reg4};
                this.SaveLoadList_mc.__get__entryList().unshift(__reg5);
            }
            else if (!this.bSaving && this.SaveLoadList_mc.__get__entryList()[0].text == __reg4) 
            {
